...
const ScheduleLookup: React.FC = () => {
  ...
  return (
    <div className="glass-card p-6 md:p-8 rounded-2xl border border-white/10 w-full space-y-8">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold text-white">State/UT Census Schedule & Portal</h3>
            <p className="text-sm text-gray-400">Check officially published timelines for your region</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
          >
            {schedules.map((s) => (
              <option key={s.statecode} value={s.statecode} className="bg-gray-900">
                {s.state} ({s.statecode})
              </option>
            ))}
          </select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <span className="text-xs text-gray-400 uppercase font-semibold">Phase 1 National Window</span>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <span className="text-xs text-gray-400 uppercase font-semibold">Phase 2 National Schedule</span>
          <p className="text-base font-bold text-white mt-1">February 2027</p>
          <p className="text-xs text-purple-400 mt-1">Population Enumeration</p>
        </div>

        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <span className="text-xs text-gray-400 uppercase font-semibold">State-Specific 30-Day Window</span>
          <p className="text-base font-bold text-amber-300 mt-1">TBD per State Notification</p>
          <p className="text-xs text-gray-400 mt-1">{selectedSchedule.notes || 'Specific state window to be notified.'}</p>
        </div>
      </div>
      ...
      <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <a
          ...
          className="bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs px-4 py-2.5 rounded-lg transition-colors flex items-center gap-1.5 shrink-0"
        >
          ...
          {steps.map((st) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <h5 className={`text-sm font-semibold ${checked ? 'text-emerald-300' : 'text-gray-200'}`}>
                    {st.title}
                  </h5>
